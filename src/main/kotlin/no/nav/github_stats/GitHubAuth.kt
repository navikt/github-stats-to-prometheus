...
fun resolveToken(
    ...
    runBlocking {
        log.info("Using GitHub App authentication (appId=${config.appId}, installationId=${config.installationId})")
        val jwt = buildJwt(config.appId, config.privateKey)
        val response =
            httpClient.post("${config.githubApiUrl}app/installations/${config.installationId}/access_tokens") {
                header(HttpHeaders.Authorization, "Bearer $jwt")
                header(HttpHeaders.Accept, "application/vnd.github+json")
                header("X-GitHub-Api-Version", config.githubApiVersion)
