...
class GitHubClient(private val httpClient: HttpClient, private val apiUrl: String, val org: String) {
    ...
    suspend fun latestCommit(repo: String): Commit? =
        safeFetch(repo, "latest commit") {
            ...
            if (!response.status.isSuccess()) {
                log.warn("HTTP ${response.status} fetching latest commit for '$repo'"); return@safeFetch null
            }
    ...
    private suspend fun <T> fetchAllPages(firstUrl: String, parse: suspend (HttpResponse) -> List<T>): List<T> {
        ...
        while (nextUrl != null) {
            ...
            if (!response.status.isSuccess()) {
                log.warn("HTTP ${response.status} fetching '$nextUrl'"); break
            }
    ...
    private suspend fun <T> safeFetch(repo: String, what: String, block: suspend () -> T): T? =
        try {
            ...
            log.warn("Failed to fetch $what for '$repo': ${e.message}"); null
        }
}

@Serializable
data class OrgRepository(val name: String, val archived: Boolean, val visibility: String, val permissions: Permissions)
@Serializable
...
@Serializable
data class PullRequest(val updated_at: String, val title: String, val user: User)
@Serializable
data class User(val login: String, val type: String)
@Serializable
data class DependabotAlert(val security_vulnerability: SecurityVulnerability)
@Serializable
...
@Serializable
data class Commit(val commit: CommitDetails)
@Serializable
data class CommitDetails(val author: CommitAuthor)
@Serializable
