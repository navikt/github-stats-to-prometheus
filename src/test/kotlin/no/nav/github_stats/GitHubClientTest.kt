...
class GitHubClientTest {
    ...
    @Test fun `teamRepos returns parsed repositories`() = runBlocking {
        val c = client { jsonResponse("""[{"name":"repo-a","archived":false,"visibility":"public","permissions":{"admin":false,"maintain":false,"push":true,"triage":false,"pull":true}}]""") }
        ...
        val c = client {
            ...
            when (page) {
                1 -> jsonResponse(
                    ...
                    headers = headersOf("Link", """<https://api.github.com/orgs/my-org/teams/team-x/repos?page=2>; rel="next"""")
                )
                else -> jsonResponse(
    ...
    @Test fun `teamRepos stops pagination on 500`() = runBlocking {
        var page = 0
        val c = client {
            page++
            when (page) {
    ...
    @Test fun `openPullRequests returns parsed PRs`() = runBlocking {
        val c = client { jsonResponse("""[{"updated_at":"2024-01-01T00:00:00Z","title":"Fix bug","user":{"login":"octocat","type":"User"}},{"updated_at":"2024-01-02T00:00:00Z","title":"Bump dep","user":{"login":"dependabot[bot]","type":"Bot"}}]""") }
        val prs = c.openPullRequests("repo-a")
        assertEquals(2, prs.size)
        assertEquals("dependabot[bot]", prs[1].user.login)
    }

    @Test fun `openPullRequests returns empty list on 403 rate limit`() = runBlocking {
    ...
    @Test fun `openPullRequests returns empty list on 429 too many requests`() = runBlocking {
        val c = client { jsonResponse("""{"message":"Too many requests"}""", HttpStatusCode(429, "Too Many Requests")) }
        assertEquals(emptyList(), c.openPullRequests("repo-a"))
    }

    @Test fun `openPullRequests returns empty list on malformed JSON`() = runBlocking {
        val c = client { jsonResponse("not json at all") }
        assertEquals(emptyList(), c.openPullRequests("repo-a"))
    }

    @Test fun `openPullRequests returns empty list on unexpected JSON shape`() = runBlocking {
        val c = client { jsonResponse("""{"unexpected":"object"}""") }
        assertEquals(emptyList(), c.openPullRequests("repo-a"))
    }

    @Test fun `dependabotAlerts parses severity correctly`() = runBlocking {
        val c = client { jsonResponse("""[{"security_vulnerability":{"severity":"critical"}},{"security_vulnerability":{"severity":"high"}},{"security_vulnerability":{"severity":"medium"}}]""") }
        val alerts = c.dependabotAlerts("repo-a")
        assertEquals(3, alerts.size)
        assertEquals("critical", alerts[0].security_vulnerability.severity)
    }

    @Test fun `dependabotAlerts returns empty list on 404`() = runBlocking {
        val c = client { jsonResponse("", HttpStatusCode.NotFound) }
        assertEquals(emptyList(), c.dependabotAlerts("repo-a"))
    }

    @Test fun `latestCommit returns parsed commit`() = runBlocking {
    ...
    @Test fun `latestCommit returns null on empty repo`() = runBlocking {
        val c = client { jsonResponse("[]") }
        assertNull(c.latestCommit("repo-a"))
    }

    @Test fun `latestCommit returns null on 409 conflict (empty git repo)`() = runBlocking {
        val c = client { jsonResponse("""{"message":"Git Repository is empty."}""", HttpStatusCode.Conflict) }
        assertNull(c.latestCommit("repo-a"))
    }

    @Test fun `latestCommit returns null on 500`() = runBlocking {
        val c = client { jsonResponse("error", HttpStatusCode.InternalServerError) }
        assertNull(c.latestCommit("repo-a"))
    }

    @Test fun `secretAlerts returns count`() = runBlocking {
        val c = client { jsonResponse("""[{},{}]""") }
        assertEquals(2, c.secretAlerts("repo-a"))
    }

    @Test fun `secretAlerts returns 0 on 404`() = runBlocking {
        val c = client { jsonResponse("", HttpStatusCode.NotFound) }
        assertEquals(0, c.secretAlerts("repo-a"))
    }

    @Test fun `codeScanningAlerts returns count`() = runBlocking {
        val c = client { jsonResponse("""[{},{},{}]""") }
        assertEquals(3, c.codeScanningAlerts("repo-a"))
    }

    @Test fun `codeScanningAlerts returns 0 on 403 (feature not enabled)`() = runBlocking {
    ...
    @Test fun `ignores unknown fields in JSON response`() = runBlocking {
        val c = client { jsonResponse("""[{"name":"repo-a","archived":false,"visibility":"public","permissions":{"admin":false,"maintain":false,"push":true,"triage":false,"pull":true},"extra_future_field":"ignored"}]""") }
        val repos = c.teamRepos("team-x")
        assertEquals(1, repos.size)
    }
}
