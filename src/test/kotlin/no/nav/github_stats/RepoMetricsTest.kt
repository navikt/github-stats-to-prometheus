...
class RepoMetricsTest {
    ...
    @Test
    fun `openDependenciesSum counts individual dependabot PRs`() {
    ...
    @Test
    fun `openDependenciesSum expands grouped dependabot PR`() {
        val metrics = RepoMetrics(
            repository = "repo",
            openPRs = 1,
    ...
    @Test
    fun `openDependenciesSum handles mix of grouped and individual PRs`() {
        val metrics = RepoMetrics(
            repository = "repo",
            openPRs = 3,
            dependabotPrs = listOf(
    ...
    @Test
    fun `criticalAlertsSum counts only critical severity`() {
        val metrics = RepoMetrics(
    ...
    @Test
    fun `highAlertsSum counts only high severity`() {
        val metrics = RepoMetrics(
            repository = "repo", openPRs = 0, dependabotPrs = emptyList(),
    ...
    @Test
    fun `criticalAlertsSum and highAlertsSum are zero with no alerts`() {
    ...
    @Test
    fun `daysSinceLatestCommit is null when no commit`() {
    ...
    @Test
    fun `daysSinceLatestCommit computes correct days`() {
        val daysAgo = 10L
    ...
    @Test
    fun `daysSinceLatestCommit is zero for today`() {
