...
class RoleTest {
    ...
    private fun perms(admin: Boolean = false, maintain: Boolean = false, push: Boolean = false, triage: Boolean = false, pull: Boolean = true) =
        Permissions(admin = admin, maintain = maintain, push = push, triage = triage, pull = pull)

    @Test fun `meetsMinimum is true for same role`() {
        Role.entries.forEach { assertTrue(it.meetsMinimum(it), "$it should meet itself") }
    }

    @Test fun `meetsMinimum respects hierarchy`() {
    ...
    @Test fun `meetsMinimum is false going downward`() {
