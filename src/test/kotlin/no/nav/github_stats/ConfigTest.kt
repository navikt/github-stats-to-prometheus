...
class ConfigTest {
    ...
    @Test fun `parses valid config with PAT`() {
        ...
        assertEquals(Role.PUSH, config.minimumRole)
        assertTrue(config.excludedRepos.isEmpty())
        assertEquals(AuthMode.Pat("ghp_test"), config.authMode)
    }

    @Test fun `trims trailing slash from API URL and adds one`() {
    ...
    @Test fun `defaults API URL to github`() {
        val config = Config.fromEnv(base)
        assertEquals("https://api.github.com/", config.githubApiUrl)
    }

    @Test fun `parses MINIMUM_ROLE case-insensitively`() {
    ...
    @Test fun `parses EXCLUDED_REPOS`() {
    ...
    @Test fun `fails fast with no auth configured`() {
    ...
    @Test fun `fails fast on invalid MINIMUM_ROLE`() {
    ...
    @Test fun `parses GitHub App auth mode`() {
    ...
    @Test fun `App auth fails fast when private key is missing`() {
        val env = base - "GITHUB_PAT" + mapOf(
            "GITHUB_APP_ID" to "123456",
            "GITHUB_APP_INSTALLATION_ID" to "78901234"
        )
    ...
    @Test fun `App auth fails fast when installation ID is not numeric`() {
        val env = base - "GITHUB_PAT" + mapOf(
            "GITHUB_APP_ID" to "123456",
    ...
    @Test fun `App auth fails fast when app ID is not numeric`() {
        val env = base - "GITHUB_PAT" + mapOf(
    ...
    @Test fun `App auth takes precedence over PAT when both present`() {
        val env = base + mapOf(
            "GITHUB_APP_ID" to "123456",
            "GITHUB_APP_PRIVATE_KEY" to "key",
            "GITHUB_APP_INSTALLATION_ID" to "78901234"
    ...
    @Test fun `dummy push gateway address is accepted`() {
